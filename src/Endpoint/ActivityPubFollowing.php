 ...
 */
final class ActivityPubFollowing implements IActivityPubEndpoint {
	...
	public function getOutput(): string {
		...
		if (preg_match('#^/users/([^/]+)/following#', $path, $m)) {
			$actorName = $m[1];
		}
		if (!$actorName) {
		...
		$followingUrl = "https://$domain/users/" . rawurlencode($actorName) . "/following";
		...
		// DB: count following (actors this user follows)
		$row = $this->database->singleQuery("
			SELECT COUNT(*) AS cnt
			FROM followers f
			WHERE f.actorId = $localActorId AND f.accepted = 1
		");
		$total = (int)$row['cnt'];

		$this->logger->info(
			"Serving following metadata for $actorName at $followingUrl ($total items)",
			['scope' => 'ActivityPub']
		);
